...
- **다중 도시 여행** — 한 여행에 **여러 도시**(도쿄 → 오사카 → 교토)를 순서·박수로 담아 **하나의 여행에서 통합 조망**. 날짜 자동 배분, 도시 전환 시 지도·일정 이동, 장소를 도시별로 관리, 경계에 **도시 간 이동(기차/항공) 카드**. 정규화(`trip_city`)·날짜 파생·전 구간 하위호환(도시 1개 = 단일 도시 여행).
...
- **여행 생성 마법사** — 다단계 폼(제목·**도시들·박수**·멤버·권한, 날짜 자동 계산). 빈 여행 또는 **템플릿 복제로 시작**(도쿄·제주 시드를 서버 RPC로 복제).
