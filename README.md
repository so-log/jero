...
### 팜플렛 PDF — 서버 headless 대신 print 폴백

여행 팜플렛(A4 3단)을 서버 headless Chromium(`puppeteer-core` + `@sparticuz/chromium`)으로 PDF 생성하려 했으나, **이 환경의 Node 20 vs 라이브러리의 node22 요구가 충돌**하고 `--ignore-engines` 강제 시 Vercel 프로덕션 빌드가 깨질 위험이 있었습니다. 배포 안전을 우선해 설계에 명시해둔 **`window.print()` 폴백**으로 전환했습니다. 인쇄 라우트가 미리보기와 **동일 컴포넌트·`faces` 레이아웃을 재사용**하므로 결과물은 일치하며, 서버 PDF 자동 다운로드는 Node 22 환경 확보 시 승격 예정입니다.

## 테스트

- **단위·통합**: Vitest + Testing Library — 125 tests. "데이터 응답 → 화면 렌더링" 통합 검증을 유닛 테스트보다 우선.
...
```bash
yarn run check      # typecheck + lint + Vitest(125 tests) — 커밋/PR 전 게이트
