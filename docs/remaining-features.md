...
## 7. AI 어시스턴트 (18) 후속 — phase 1~5 구현 완료 후 남은 갭
...
- 기획 §6 "LLM 호출 실패·타임아웃 → 안내 + **재시도 버튼**" 중 **안내만** 구현됐다. 설계 §9 의 `MessageActions.tsx` 미구현.
- 마지막 사용자 메시지를 재전송하는 얇은 컴포넌트로 충분하다(rate limit 1회 소비 — 기획 §5 7항).

### K. 코스 푸터 시안 정합 여부 결정

- 시안·기획 §11 은 `코스 적용` + `동선 최적화` **병치**인데, 구현은 적용 후 순차 노출이다(설계 §13.1-4, 의도적 이탈).
- 선택지: ① 현재 동작 유지 + **기획·시안 문구를 구현에 맞게 정정** ② 병치로 되돌리고 적용 전 `동선 최적화` 를 비활성 노출. **①을 권장**(적용 전에는 최적화 대상이 없다).

### L. 어시스턴트 경로 viewer 쓰기 거부 회귀 테스트

- `place` RLS(`editor+`)가 구조적으로 막고 UI 미노출도 검증되지만, **어시스턴트 경로 전용 DB 테스트는 없다**. 기획 §11 "강제로 호출해도 서버가 거부" 를 테스트로 고정하려면 viewer 세션으로 `place` insert 를 시도해 거부를 확인하는 케이스를 추가한다.

### M. 후속(설계에 있으나 MVP 범위 밖)

- **대화 영속화** — 계약 C9 #2 로 **비영속 확정**. 영속화 시 보관기간·삭제 UI 동반 필요.
- **진입 뷰 확장** — 현재 `plan`·`places` 만(계약 C9 #5). `calendar`·`budget`·`stats` 는 후속.
- **`getTripSnapshot` 도구** — 컨텍스트 주입으로 대체해 의도적으로 만들지 않았다(설계 §5.1). 대화 중 최신 상태 재조회 요구가 생기면 추가.
- **팜플렛 AI 테마 추천** — 위 §6 에 "LLM API 키 필요 — 보류"로 적혀 있었는데, **키는 이제 있다**(어시스턴트와 공용 `GOOGLE_GENERATIVE_AI_API_KEY`). 착수 가능해진 항목.
