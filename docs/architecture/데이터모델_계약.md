...
- **RPC 4종 구현 상태**(C5 3종 + C6 1종): `match_place_embeddings` ✅ 사용 중(`lib/ai/retrieve.ts`) · `consume_assistant_quota` ✅ 사용 중(챗 라우트 rate limit) · `upsert_place_embedding`·`stale_place_embeddings` ✅ 구현(`POST /api/assistant/index`)되었으나 **라우트 호출부가 아직 없다** → `place_embedding` 은 실사용에서 비어 있다. 계약·DB 는 완결이고 **배선만 남았다**(설계 §3.3, `docs/remaining-features.md`).
