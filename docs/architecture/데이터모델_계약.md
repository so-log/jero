...
- **`set search_path = public` 고정**: definer 함수는 호출자가 `search_path` 를 조작해 동명의 가짜 함수/테이블(`is_trip_member` 등)을 심는 공격이 가능하다. 신규 definer 함수 2개(`upsert_place_embedding`·`consume_assistant_quota`)에는 **`set search_path = public` 을 명시**한다.
  > 기존 definer 함수(`create_trip`·`accept_invite`·`get_shared_trip`, B2.1·B6)에는 이 설정이 없다. **이 PR 범위 밖**이므로 건드리지 않되, **후속 하드닝 항목**으로 남긴다(`docs/remaining-features.md` 후보).
...
- **후속 하드닝(범위 밖, 기록만)**: 기존 definer 함수 `create_trip`·`accept_invite`(B2.1)·`get_shared_trip`(B6)에 `set search_path = public` 미설정 — 별도 PR 로 처리 권장.
