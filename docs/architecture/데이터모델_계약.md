...
**결정: (B)**. 호출자는 `place_id` 와 벡터만 넘기고, **`trip_id`·`content` 는 함수가 `place` 행에서 직접 읽어 채운다** → 클라이언트가 조작할 수 있는 필드가 사라진다. 함수 내부에서 `is_trip_member` 를 직접 검증하므로 정의자 권한이 남용으로 이어지지 않는다(B2.1 과 같은 규율).
...
- **차원: 768** (`outputDimensionality: 768`). 이 모델은 MRL(Matryoshka)로 3072(기본)/1536/768 을 지원한다.
  ...
  - **품질**: 임베딩 원문이 짧다(장소명+카테고리+지역+메모 ≈ 30~60토큰). 768 로 충분.
...
| `content` | text not null | 임베딩 원문 = `name` + `category` + `area` + `memo`. **RPC 가 place 행에서 생성**(C1) |
...
begin
  ...
  -- 정의자 권한이지만 자격은 직접 검증(B2.1 규율). 멤버면 역할 무관(viewer 도 인덱싱 가능).
...
returns text language sql immutable as $$
  select trim(both ' ' from concat_ws(' · ',
    p.name, p.category::text, nullif(p.area,''), nullif(p.memo,'')));
$$;
...
- `match_place_embeddings` 는 **`security invoker`** 다 — 정의자 권한이면 RLS 를 우회해 남의 여행을 검색할 수 있다. 호출자 권한이므로 `pe_select` 정책이 그대로 걸린다(비멤버는 0행).
...
```jsonc
[
  {"place_id":"p1","content":"츠키지 장외시장 · food · 츠키지 · 아침 스시","similarity":0.82},
...
- **검토 반영**: ★수정1 임베딩 쓰기 = `security definer` RPC 채택(직접 쓰기 정책 없음, `trip_id`·`content` 서버 파생) · ★수정2 Gemini 768차원 확정(3072 은 pgvector 인덱스 2000 한계로 배제) · ivfflat 폐기 → 정확 검색 + HNSW 승격 임계치 · Places 서버키 IP→**API 제한** · 열린질문 1~5 확정.
