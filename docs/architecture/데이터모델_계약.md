...
> ✅ **적용 완료** — `supabase/migrations/0008_assistant.sql` 이 phase 1(#31)에서 생성됐다. 아래는 그 초안이며, 실제 파일과 다를 경우 **마이그레이션 파일이 기준**이다.
...
## C10. GATE 2 (AI 어시스턴트 설계) — ✅ 승인 완료 · 구현 완료
...
- **승인 후 구현**: ✅ 설계 §13 phase **1~5 완료**(#31·#32·#33·#34·#35), 각 단계 독립 PR·회귀 0.
- **마이그레이션**: ✅ `supabase/migrations/0008_assistant.sql` **생성·적용 완료**(phase 1). 계약 예시(Part C4~C8)와 실제 `.sql` 이 어긋나면 **마이그레이션 파일이 기준**이다(C10 위 정정 주석과 동일 원칙).
- **RPC 4종 구현 상태**(C5 3종 + C6 1종): `match_place_embeddings` ✅ 사용 중(`lib/ai/retrieve.ts`) · `consume_assistant_quota` ✅ 사용 중(챗 라우트 rate limit) · `upsert_place_embedding`·`stale_place_embeddings` ✅ 사용 중(`POST /api/assistant/index` — 워크스페이스 진입 시 `useIndexPlaces` 가 호출, `feat/assistant-index-wiring`). 실 Supabase e2e 로 `place_embedding` 이 채워지고 유사 검색이 근거로 쓰이는 것까지 확인(설계 §3.3).
