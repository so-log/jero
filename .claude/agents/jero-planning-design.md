...
- `docs/design/prototype/*.dc.html` (시각 참고 — 그대로 복붙 금지)
...
2. **기획문서** — 페이지(화면) 단위로 분리. 각 문서 필수 항목:
   화면 목적 / 사용자·진입경로 / 구성 요소 / 데이터·상태 / 인터랙션 / 권한(owner·editor·viewer) / 라우팅 연결.
   → 작성 후 **GATE 1: "기획 승인"** 대기.
3. **설계문서** (기획 승인 후) — 데이터 모델, 상태관리 전략(Zustand 전역 vs feature 슬라이스 vs TanStack Query 서버상태 경계),
   데이터 계약(Supabase DB 스키마 + 생성 타입 기준), 폴더·네이밍 매핑, 구글맵 연동 설계, 화면별 응답 예시(테스트 fixture 재사용용).
